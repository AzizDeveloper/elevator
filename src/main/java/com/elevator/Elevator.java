...
public class Elevator {
    ...
    //the current floor of the elevators. By default - first floor
        ...
        while (checkPeople(building.getFloors(), currentPeopleCount)) {
            ...
            if (direction.equals("UP")) {
                ...
                if (currentFloor == maxFloor) {
                    moveElevatorIfOnTheLastFloor("DOWN", floor, people);
                    continue;
                }
            ...
            //moves the elevator down and decrements floor after the move
                ...
                if (currentFloor == 1) {
                    moveElevatorIfOnTheLastFloor("UP", floor, people);
                    continue;
                }
                moveElevator(floor, people);
                currentFloor--;
    ...
    /**
     * The elevator moves while there are people inside it or there are people who didn't reach the needed floor
     *
     * @param floors             current floor
     * @param currentPeopleCount current people inside of elevator
     * @return true if there are people, false if there are no people inside the elevator or not everybody reached the needed floor
     */
    private boolean checkPeople(List<Floor> floors, int currentPeopleCount) {
        int result = 0;
        for (Floor floor : floors) {
            result += floor.getPeopleOnTheFloor()
                    .stream()
                    .filter(e -> e.getCurrentFloor() != e.getNextFloor()).count();
        }
    ...
    private int recalculateMaxFloor() {
        List<Integer> nextFloorNumbers = peopleInElevator
                .stream()
    ...
    private void movePeopleToElevator(Set<Person> people, Floor floor) {
        for (Person person : people) {
            ...
            if (currentPeopleCount < maxPeopleCount) {
                ...
                floor.getPeopleOnTheFloor().remove(person);
                currentPeopleCount++;
            }
        }
    }
    ...
    private Set<Person> handleFloor(String direction, Floor floor) {
        ...
        return floor.getPeopleOnTheFloor()
                .stream()
                .filter(e -> e.getDirection().equals(direction) && e.getCurrentFloor() != e.getNextFloor())
                .collect(Collectors.toSet());
    }
     ...
     */
    private void moveElevatorIfOnTheLastFloor(String direction, Floor floor, Set<Person> people) {
